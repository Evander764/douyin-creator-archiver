...
```bash
dyca search-keywords --keywords-file ./presets/business-keywords.txt --out ./douyin-keyword-search --target-per-keyword 10 --max-scanned-per-keyword 200 --min-red-hearts 1000 --within-days 7
```
...
- Keyword time window: rolling 7 days; missing timestamps are excluded

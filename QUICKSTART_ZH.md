...
程序只复用一个抖音搜索窗口：在搜索框里逐字填入 `#关键词`，读回确认后点击“搜索”；切词时先全选、删除旧词并确认输入框为空，再输入新词。每个词找到 1 条合格内容后切换；扫描到 200 条仍没有合格内容也切换。合格内容必须同时满足：`statistics.digg_count > 1000`，并且 `create_time` 位于运行时点往前 60×24 小时内。运行结束后保留抖音窗口，避免反复开关引起风控，同时把前台焦点恢复到运行前的应用。

传入 `--qualified-hook` 后使用流式并行模式：命中内容时复制详情为后台备份标签页，原标签页立即通过浏览器历史回到搜索页继续刷；后台 worker 独立处理下载、数据入库、转录和音频清理。默认并发 2 个 worker，可用 `--hook-concurrency 1..8` 调整。搜索完成后程序会等待全部 worker 收尾，并输出 `pipeline-report.json`。

需要“发现一条就入库一条”时，加 `--qualified-hook /绝对路径/入库脚本.mjs`。程序会复制详情标签页并立刻让原标签页回到搜索结果；入库脚本进入后台 worker，不阻塞后续搜索。回退校验失败会立即停止搜索；worker 失败会记录在 `pipeline-report.json`，其他搜索仍继续，最终状态标为 `partial_failure`。入库后不会重新搜索当前关键词。

...
程序只复用一个抖音搜索窗口：在搜索框里逐字填入 `#关键词`，读回确认后点击“搜索”；切词时先全选、删除旧词并确认输入框为空，再输入新词。每个词找到 3 条合格内容后切换；扫描到 200 条仍不足 3 条也切换。合格内容必须同时满足：`statistics.digg_count > 1000`，并且 `create_time` 位于运行时点往前 14×24 小时内。运行结束后保留抖音窗口，避免反复开关引起风控，同时把前台焦点恢复到运行前的应用。
